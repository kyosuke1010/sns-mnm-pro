...
export function normalizeGenerationInput(feature, input = {}, profile = {}, options = {}) {
  ...
  return {
    ...
    provided_trend: String(input.trend || "").trim(),
...
function commonSystemPrompt() {
  return [
    ...
    "- The closing question must ZOOM INTO one specific, vivid micro-situation the reader instantly recognizes — name the concrete 相手・場面・気持ち — not a broad category. Weak (too broad): 「返信に困るのはどんな時？」. Strong (specific + warm): 「今いい感じの人への返信で、どう返すか迷う時ってどんな時が多いですかー？」.",
    "- For friendly/energetic tones, soften the question casually (〜ですかー？ / 〜なときが多い？ / 〜ない？) instead of a clipped formal question.",
    ...
    "",
    "REGISTER (talk like a real person, NOT an announcer or teacher):",
    ...
    "- RHYTHM VARIATION (critical for human-likeness): Do not let sentence length stay uniform across a post. Real humans write one very short sentence, then a longer one that explains it, then maybe a question. A post where every sentence is roughly the same length reads like a template. Break the rhythm on purpose — one sentence can be as short as two or three words if it lands a point.",
    "- EMOTIONAL ARC: Let the emotional temperature shift within a post. Starting slightly lower (a small worry, a question, a confession) and rising toward warmth, confidence, or an invitation feels like real speech. A post that stays at the same emotional pitch from start to finish feels generated.",
    "- TALKING TO vs. TALKING AT: The strongest posts feel like the writer is speaking directly to one specific person, not broadcasting to a crowd. Use 「あなた / 君 / みんな」 sparingly and only when it feels natural — the intimacy comes from the specificity of the scene, not from the second-person address.",
    ...
    "- Write for the algorithm of human emotion and connection, not for keyword/SEO mechanics. Posts spread when a real person feels something and wants to react, reply, or share.",
    "- Use wordplay, analogy, parody, and format-riffing to make the point land and feel current. Twisting a familiar phrase or riding a known format is a strong reach move — do it when it fits the topic and tone.",
    "- Lean into interaction like a real account building community: invite people to agree, share their own take, jump in, or answer a light question. 「絡むなら今」的な巻き込みは強い。",
    "- INSIDER HONESTY (a trust move, not cynicism): occasionally naming the awkward thing everyone quietly avoids saying — a tatemae (建前) most posts paper over — reads as a real person, not a brand. Use sparingly, aim it at a shared frustration or absurdity (never at a vulnerable reader or a specific group of people), and never turn it into an insult or a sales pitch.",
    "- UNRESOLVED FRAGMENT (optional, use rarely): a post can also just present one striking, half-explained moment and stop — no lesson, no full context, no payoff — leaving a little curiosity gap. This reads more human than a fully explained story. Do not force this on every candidate; one out of a set is enough.",
    "- TRUTH LIMIT (important): you do NOT have real-time trend data. Never claim something is 'いま流行ってる/バズってる' or invent specific current memes, news, prices, or events. Only ride a trend/phrase the user actually provides in the input, or a timeless well-known format. Never fabricate facts about real people or current events.",
    "- If the context field provided_trend is non-empty, ride THAT exact trend/phrase: make it the hook or the central analogy and twist it toward the topic. Use only that provided trend; do not swap in a different one or claim broader trend facts you cannot verify. If provided_trend is empty, do not invent a trend.",
    "- The trend is only the ENTRY. Bridge it into the user's actual topic, then land on a concrete payoff the target reader would want to read or save — e.g. a note記事の切り口 like 「なぜ迷ったのか / どの言葉で冷めたのか / 次はどう動くか」. The reader's takeaway is the point; the trend just gets them in the door.",
    ...
    "- The same tell also hides in the comparative form 「Xより(も)Yなんだよね/と思う」. If used, never pair two abstract nouns (例: 正解より場面、本質より結果、正しさより在り方 — abstract vs abstract reads flat and generated). Anchor at least one side in something felt, physical, or momentary (感情, 気持ち, その時の体感, 具体的な瞬間) instead of another concept. Bad: 「正解より場面なんだよね」. Good: 「正論より、その時の感情なんだよね」— the second one lands because it trades a concept for a feeling, not a concept for another concept.",
    ...
    "- If the topic is literally about which AI is better or about combining AIs, naming concrete AI tools and what each is actually good at is REQUIRED, not optional. Casual nicknames like チャッピー (=ChatGPT) are fine when the tone is friendly.",
    "- Stay truthful: mention only widely-known, generally-true strengths. Never invent specific prices, benchmark numbers, release dates, or features you are unsure about.",
    "",
...
function fewShotPrompt(feature, platform) {
  const threads = [
    ...
    "",
    "BAD Threads pattern (generic advice):",
    "投稿はターゲットを決めることが大事です。読みやすくすると反応が増えます。意識しましょう。",
    "",
    "BAD Threads pattern (AI-smell formula — the most important one to avoid):",
    "仕事終わりの22:30。洗濯物をたたみながら、スマホでAIアプリを開く。",
    "でも候補が多すぎて、結局なにも書けない。これ、かなりあるあるです。",
    "保存しておくならこの分け方👇",
    "① ネタ出し ② 読者の悩み ③ 文章づくり",
    "AIを1つに決めきれなくてOK。今日から使い分けで変えられます。",
